...
public:
  ...
  bool begin() {
    Wire.begin(_sda, _scl);          // ESP8266 Wire 无第三参数
    Wire.setClock(_freq);
    _addr = detectAddress();
    return _addr != 0;
  }
...
private:
  ...
  uint16_t readWord(uint8_t reg) {
    // ESP8266 软件 I2C 会被 WiFi 中断打断, 偶发读错。
    // 两次读数相近(环形差 <=8 LSB)才接受: 两次读间隔仅几 ms, 转动中真实变化
    // 极小, 干扰错误值才是大跳变 —— 相等校验会误杀转动中的读数。
    for (int attempt = 0; attempt < 3; attempt++) {
      uint16_t v = rawReadWord(reg);
      if (v == 0xFFFF) return 0xFFFF;
      uint16_t v2 = rawReadWord(reg);
      if (v2 == 0xFFFF) return 0xFFFF;
      int16_t diff = v - v2;
  ...
  uint16_t rawReadWord(uint8_t reg) {
    Wire.beginTransmission(_addr);
    Wire.write(reg);
